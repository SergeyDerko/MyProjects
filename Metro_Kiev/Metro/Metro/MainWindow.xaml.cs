...
namespace Metro
{
    public partial class MainWindow
    {
        ...
        private List<Ellipse> Run(Ellipse start, Ellipse stop)
        {
            ...
            {
                qvery.Add(ПалацСпорту);qvery.Add(G19);qvery.Add(G18);qvery.Add(ЗолотіВорота);
                return qvery;
            }
            if (Equals(_stop, ПлощаЛьваТолстого) && Equals(_start, Театральна))
            {
                qvery.Add(ЗолотіВорота);qvery.Add(G18);qvery.Add(G19);qvery.Add(ПалацСпорту);
                return qvery;
            }
            if (Equals(_start, ЗолотіВорота) && Equals(_stop, МайданНезалежності))
            {
                qvery.Add(Театральна); qvery.Add(R16); qvery.Add(Хрещатик);
                return qvery;
            }
            if (Equals(_stop, ЗолотіВорота) && Equals(_start, МайданНезалежності))
            {
                qvery.Add(Хрещатик); qvery.Add(R16);qvery.Add(Театральна);
                return qvery;
            }
            if (Equals(_start, ПалацСпорту) && Equals(_stop, Хрещатик))
            {
                qvery.Add(ПлощаЛьваТолстого);qvery.Add(B17); qvery.Add(B16);qvery.Add(МайданНезалежності);
                return qvery;
            }
            if (Equals(_stop, ПалацСпорту) && Equals(_start, Хрещатик))
            {
                qvery.Add(МайданНезалежності); qvery.Add(B16); qvery.Add(B17); qvery.Add(ПлощаЛьваТолстого);
                return qvery;
            }
